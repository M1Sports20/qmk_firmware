...
#ifdef NEW_OLED_DRIVER_ENABLE

void keyboard_pre_init_user(void)
{
    painter_device_t lcd_dev = qp_ssd_i2c_make_device(OLED_SPI_RST, 0x3c);

    qp_init(lcd_dev, QP_ROTATION_0);

    qp_rect(lcd_dev, 0,  0, 127, 31, 0, 0, 1, false);
    qp_line(lcd_dev, 0,  0, 127, 63, 0, 0, 1);
    qp_line(lcd_dev, 0, 63, 127,  0, 0, 0, 1);

    qp_flush(lcd_dev);
}

#endif /* NEW_OLED_DRIVER_ENABLE */
